...
cat > src/routes/property.routes.js << 'EOF'
...
cat > src/routes/maintenance.routes.js << 'EOF'
const express = require('express');
const router = express.Router();
...
cat > src/routes/document.routes.js << 'EOF'
const express = require('express');
const router = express.Router();
const { authMiddleware } = require('../middleware/auth.middleware');
...
cat > src/routes/pathology.routes.js << 'EOF'
const express = require('express');
const router = express.Router();
const { authMiddleware } = require('../middleware/auth.middleware');
...
cat > src/routes/notification.routes.js << 'EOF'
const express = require('express');
const router = express.Router();
const { authMiddleware } = require('../middleware/auth.middleware');

router.use(authMiddleware);
router.get('/', (req, res) => res.json({ message: 'Listar notificações' }));

module.exports = router;
EOF

cat > src/controllers/maintenance.controller.js << 'EOF'
